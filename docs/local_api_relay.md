...
### 当前 live 盘点（2026-04-16）

当前这台机器上的 live relay 仍然跑在 root 的 `systemd --user` unit 下：

- unit 名：`local-api-relay.service`
- unit 文件：`/root/.config/systemd/user/local-api-relay.service`
- `WorkingDirectory`：`/root/.openclaw/workspace/relay`
- `ExecStart`：`/usr/bin/python3 /root/.openclaw/workspace/relay/local_api_relay.py --config /root/.openclaw/workspace/relay/local_api_relay.json`
- `Environment`：`HOME=/root TMPDIR=/tmp PYTHONDONTWRITEBYTECODE=1`

迁移原因已经确认：

- `sudo -n bash deploy/bin/relay-admin status` 在现网会报 `Failed to connect to bus: No medium found`
- 当前 root shell 里虽然有 `XDG_RUNTIME_DIR=/run/user/0`，但 `DBUS_SESSION_BUS_ADDRESS` 为空
- 所以 `relay-admin` 继续依赖 `systemctl --user` 不稳定，统一运维入口必须改为标准 system service

### 标准 system service 模板
...
- `/etc/systemd/system/local-api-relay.service`

模板默认假设仓库路径是 `/root/.openclaw/workspace/relay`。如果实际部署路径不同，先改模板里的 `WorkingDirectory` 与 `ExecStart`。

这份模板现在明确面向 system service：

- `WantedBy=multi-user.target`
- `ExecStart` 显式使用 `serve --config`
- 不再依赖 user bus / `XDG_RUNTIME_DIR`
...
USER_UNIT_PATH=/root/.config/systemd/user/local-api-relay.service
USER_UNIT_BACKUP="/root/.config/systemd/user/local-api-relay.service.bak-$(date +%Y%m%d-%H%M%S)"

if [ -f "$USER_UNIT_PATH" ]; then
  install -m 0644 "$USER_UNIT_PATH" "$USER_UNIT_BACKUP"
fi
echo "user unit backup: $USER_UNIT_BACKUP"

systemctl --user disable --now local-api-relay.service
sudo install -m 0644 "$REPO_ROOT/deploy/systemd/local-api-relay.service" /etc/systemd/system/local-api-relay.service
sudo systemctl daemon-reload
sudo systemctl enable --now local-api-relay.service

sudo install -d -m 0755 /usr/local/bin
sudo ln -sfn "$REPO_ROOT/deploy/bin/relay-admin" /usr/local/bin/relay-admin

sudo -n relay-admin status
sudo -n relay-admin restart
```

迁移完成后的统一运维入口只有两层：

- 权威源码：`deploy/bin/relay-admin`
- 机器安装点：`/usr/local/bin/relay-admin`（symlink，不复制）

### 最小回滚步骤

如果 system service 版本需要回滚到迁移前的 user service，在同一个 root shell 中执行：

```bash
sudo systemctl disable --now local-api-relay.service
sudo mv /etc/systemd/system/local-api-relay.service \
  "/etc/systemd/system/local-api-relay.service.bak-$(date +%Y%m%d-%H%M%S)"
sudo systemctl daemon-reload

install -m 0644 "$USER_UNIT_BACKUP" /root/.config/systemd/user/local-api-relay.service
systemctl --user daemon-reload
systemctl --user enable --now local-api-relay.service
systemctl --user --no-pager --full status local-api-relay.service
```

注意：

- 这个回滚只恢复服务托管方式
- `USER_UNIT_BACKUP` 用迁移时打印出来的那个备份路径；如果换过 shell，先手工重新设置它
- 当前仓库里的 `relay-admin` 已经只面向 system service；如果你连 operator 行为也要一起回滚，需同时把 relay 仓回退到迁移前版本再重新安装脚本
...
- `/usr/local/bin/relay-admin` 只保留 symlink，不保留独立副本
